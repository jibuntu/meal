...
impl Kijun {
    pub fn new(age: usize,
        ...
        if let Ok(energy_val) = Kijun::get_energy(weight, height, age, gender, pal) {
            ...
            if let Ok(niacin) = Kijun::get_niacin(age, energy_val) {
    ...
    // ビタミンB1
    pub fn get_vitamin_b1(age: usize, energy: f32) -> Result<KijunValue, String> {
        let result = match age {
            0 => {
                return Err("0歳以下はビタミンB1の推奨量を求めることができません".to_string())
            },
            age if 1 <= age => {
                // https://www.mhlw.go.jp/file/05-Shingikai-10901000-Kenkoukyoku-Soumuka/0000114399.pdf - 196 page
                ((energy / 1000.0) * 0.45) * 1.2
            },
            _ => {
    ...
    // ナイアシン
    pub fn get_niacin(age: usize, energy: f32) -> Result<KijunValue, String> {
        let result = match age {
            0 => {
                return Err("0歳以下はナイアシンの推奨量を求めることができません".to_string())
            },
            age if 1 <= age => {
                // https://www.mhlw.go.jp/file/05-Shingikai-10901000-Kenkoukyoku-Soumuka/0000114399.pdf - 203 page
                ((energy / 1000.0) * 4.8) * 1.2
            },
            _ => {
